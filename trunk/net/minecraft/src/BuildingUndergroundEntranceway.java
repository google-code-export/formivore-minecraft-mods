...
public class BuildingUndergroundEntranceway extends Building{
	...
	public boolean build() throws InterruptedException{
		for(; bLength<WORLD_HEIGHT-j0; bLength++){
			...
			if(j0+bLength>WORLD_HEIGHT/2-10 && isArtificialWallBlock(0,bLength+PASSAGE_HEIGHT,bLength)) return false;
			
			if(j0+bLength>WORLD_HEIGHT/2 && (IS_WALLABLE[getBlockIdLocal(0,bLength,bLength)] || IS_WALLABLE[getBlockIdLocal(PASSAGE_WIDTH-1,bLength,bLength)])){
				bLength--;
				break;
			}
		}
